...
- [ ] flaky test: engine TestGetValidatedRelatedSearches_PrefersValidatedGoodTerms passes in isolation and in its own package but intermittently fails under parallel `go test ./...` — shared package-level related-term cache plus map-iteration nondeterminism in GetValidatedRelatedSearches (top-8 selection can drop the good term). Needs deterministic ordering or per-test cache isolation.

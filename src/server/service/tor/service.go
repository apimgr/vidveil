...
type TorService struct {
	...
	configDir string
	// {log_dir} — Tor writes its notice log to {log_dir}/tor.log
	logDir string
	...
	// Hidden service state — the .onion key/hostname are generated and
	// persisted by Tor itself under {data_dir}/tor/site (HiddenServiceDir);
	// privateKey/publicKey are loaded read-only from that directory for display.
	onionAddress string
	privateKey   ed25519.PrivateKey
	publicKey    ed25519.PublicKey

	// torBackendPort is the dedicated PROXY-protocol-aware loopback port the
	// hidden service forwards to (torrc HiddenServicePort). Never persisted —
	// a fresh random 64000-64999 port is allocated on every process start.
	torBackendPort int
	// proxyListener is the PROXY-protocol-aware net.Listener bound to
	// 127.0.0.1:{torBackendPort}, served through the same handler chain as
	// clearnet traffic via GetProxyListener().
	proxyListener net.Listener

	// serverPort is the HTTP server's listening port (clearnet); unrelated to
	// the Tor backend port, kept for informational/logging purposes.
	serverPort int
...
// SetLogDir sets the log directory for Tor (per PART 31: {log_dir}/tor.log)
// Should be called with {log_dir} before Start()
func (s *TorService) SetLogDir(dir string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.logDir = dir
}
...
func (s *TorService) Start(ctx context.Context, serverPort int) error {
	...
	// Allocate the dedicated PROXY-protocol-aware loopback listener the hidden
	// service will forward to (torrc HiddenServicePort). Never persisted — a
	// fresh random 64000-64999 port is chosen on every process start.
	proxyLn, torBackendPort, err := newTorBackendListener()
	if err != nil {
		s.status = TorServiceStatusError
		return fmt.Errorf("failed to allocate tor backend listener: %w", err)
	}

	// Generate torrc from config and write to configDir (per PART 31).
	// The hidden service itself is declared HERE via HiddenServiceDir/
	// HiddenServicePort/HiddenServiceExportCircuitID — NOT via ADD_ONION.
	// Regenerated every startup: torrc is derived state from TorConfig plus
	// the current backend port; the .onion identity persists via the keys
	// under HiddenServiceDir, so overwriting torrc is always safe.
	if s.configDir == "" {
		proxyLn.Close()
		s.status = TorServiceStatusError
		return fmt.Errorf("tor config directory not set, cannot write torrc")
	}
	torrcFile := filepath.Join(s.configDir, "torrc")
	// Per PART 31 storage table, Tor logs to {log_dir}/tor.log — never a hardcoded path
	torLogFile := ""
	if s.logDir != "" {
		torLogFile = filepath.Join(s.logDir, "tor.log")
	}
	torrcContent := buildTorrc(s.torConfig, siteDir, torBackendPort, torLogFile)
	if _, err := ensureTorrc(torrcFile, torrcContent); err != nil {
		proxyLn.Close()
		s.status = TorServiceStatusError
		return fmt.Errorf("failed to write torrc: %w", err)
	}
	s.logger.Info("Regenerated torrc", map[string]interface{}{"path": torrcFile, "backend_port": torBackendPort})
	...
	startConf := &tor.StartConf{
		...
		// Use custom torrc (declares the hidden service, controls SocksPort, etc.)
	...
	// The hidden service is declared entirely via torrc (HiddenServiceDir/
	// HiddenServicePort/HiddenServiceExportCircuitID) — Tor itself generates
	// and persists the v3 ed25519 key + hostname under siteDir once the
	// network is enabled and the service is published. Read it back here.
	...
	if err != nil {
		...
		proxyLn.Close()
		s.status = TorServiceStatusError
		return fmt.Errorf("failed to read onion hostname: %w", err)
	}
	s.onionAddress = strings.TrimSpace(string(onionBytes))

	// Best-effort: load the Tor-generated key pair from disk purely for
	// display (GetPublicKeyHex); never fatal if this fails.
	if err := s.loadKeysFromDisk(siteDir); err != nil {
		s.logger.Warn("Failed to load Tor-generated key pair for display", map[string]interface{}{"error": err.Error()})
	}

	s.torBackendPort = torBackendPort
	...
	s.logger.Info("Hidden service started", map[string]interface{}{
		...
		"target":        fmt.Sprintf("127.0.0.1:%d", torBackendPort),
	})
...
func (s *TorService) Stop() error {
	...
	// Close the dedicated PROXY-protocol loopback listener the hidden service
	// forwarded to; never persisted, so it is fully torn down on every stop.
	if s.proxyListener != nil {
		if err := s.proxyListener.Close(); err != nil {
			s.logger.Warn("Error closing tor proxy listener", map[string]interface{}{"error": err.Error()})
		}
		s.proxyListener = nil
	}
	s.torBackendPort = 0
...
// loadKeysFromDisk loads the Tor-generated v3 ed25519 hidden-service key pair
// from siteDir purely for read-only display (GetPublicKeyHex). Unlike
// loadOrGenerateKeys, this never generates or writes keys — Tor itself owns
// key generation for the torrc-declared hidden service.
func (s *TorService) loadKeysFromDisk(siteDir string) error {
...
// GetProxyListener returns the dedicated PROXY-protocol-aware loopback
// listener the hidden service forwards to (torrc HiddenServicePort), for the
// caller to serve through the same handler chain as clearnet traffic. Nil
// when Tor is not running (no binary found, or Start has not succeeded).
func (s *TorService) GetProxyListener() net.Listener {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.proxyListener
}
...
// buildTorrc generates a torrc configuration file content from TorConfig
// settings plus the hidden-service directory and dedicated backend port.
// Per PART 31: NEVER uses default ports 9050/9051.
// The hidden service itself is declared HERE via HiddenServiceDir/
// HiddenServiceVersion/HiddenServicePort/HiddenServiceExportCircuitID —
// NOT via the ADD_ONION control command. Tor generates and persists the
// v3 ed25519 key + hostname file under hsDir on its own.
// ControlPort is explicit `127.0.0.1:auto` — never a fixed/default port.
func buildTorrc(cfg *config.TorConfig, hsDir string, torBackendPort int, logFile string) string {
	if cfg == nil {
	...
	// Virtual port for the hidden service (what .onion visitors connect to)
	virtualPort := 80
	if cfg.VirtualPort > 0 {
		virtualPort = cfg.VirtualPort
	}

	// Tor log file per AI.md PART 31 storage table: {log_dir}/tor.log
	var logConfig string
	if logFile != "" {
		logConfig = fmt.Sprintf("\n# Tor log file — always {log_dir}/tor.log, never a hardcoded path\nLog notice file %s", logFile)
	}
...
# ControlPort uses 127.0.0.1:auto — never a fixed/default control port
# ============================================================

# Control port: always localhost auto, never a fixed/default port
...
# Hidden service — declared via HiddenServiceDir/HiddenServicePort, not
# ADD_ONION. Tor generates/persists the v3 ed25519 key + hostname here.
# HiddenServiceExportCircuitID prepends an HAProxy PROXY protocol v1
# header (opaque per-circuit ID, not a real client IP) to every backend
# connection, parsed by the app's dedicated loopback listener.
HiddenServiceDir %s
...
// ensureTorrc (re)writes the torrc file every startup. Unlike a persistent
// config file, torrc here is fully derived state — the .onion identity
// persists via the v3 ed25519 keys under HiddenServiceDir, not via torrc
// content, so regenerating and overwriting torrc on every start is always
// safe (and required, since the backend port changes every process start).
// Returns true if the file did not previously exist.
...
// newTorBackendListener allocates the dedicated PROXY-protocol-aware
// loopback listener the hidden service forwards to (torrc HiddenServicePort).
// Never persisted to server.yml — a fresh random port in 64000-64999 is
// selected on every process start, matching the server's own random-port
// scheme. The returned listener parses the HAProxy PROXY protocol v1 header
// Tor prepends to every backend connection (HiddenServiceExportCircuitID).
func newTorBackendListener() (net.Listener, int, error) {
	const minPort = 64000
	const maxPort = 64999
	...
	for i := 0; i < maxAttempts; i++ {
		...
		if err != nil {
			return nil, 0, fmt.Errorf("failed to select random tor backend port: %w", err)
		}
		...
		return &proxyproto.Listener{Listener: ln}, port, nil
	}
	return nil, 0, fmt.Errorf("failed to find an available tor backend port after %d attempts: %w", maxAttempts, lastErr)
}

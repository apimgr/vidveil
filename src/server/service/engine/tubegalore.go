...
func searchTTCache(ctx context.Context, e *BaseEngine, url string) ([]model.VideoResult, error) {
	...
	doc.Find("div[data-public-id]").Each(func(i int, s *goquery.Selection) {
		...
		if title == "" {
			// Fall back to the thumbnail's alt text, matching the
			// genericSearch title chain in helpers.go
			title = parser.ExtractAttr(img, "alt")
		}
		if title == "" {
			title = parser.CleanText(link.Text())
		}
		if title == "" {
			return
		}

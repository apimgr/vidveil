...
function showToast(message, type) {
    ...
    // Errors and warnings interrupt; success/info announce politely (PART 30)
    ...
    // Countdown bar; CSS pauses its animation on hover, JS syncs the timer
    if (dismissMs > 0) {
        var progress = document.createElement('div');
        progress.className = 'toast-progress';
        progress.style.animationDuration = dismissMs + 'ms';
        toast.appendChild(progress);
    }
    container.appendChild(toast);
    setTimeout(function() { toast.classList.add('show'); }, 10);
    // Clicking anywhere on the toast dismisses it per AI.md PART 16
    toast.addEventListener('click', function() { dismissToast(toast); });
    if (dismissMs > 0) {
        var dismissTimer = setTimeout(function() { dismissToast(toast); }, dismissMs);
        ...
        toast.addEventListener('mouseleave', function() {
            dismissTimer = setTimeout(function() { dismissToast(toast); }, dismissMs);
        });
    }
    return toast.id;
}

// Accepts a toast element or the id returned by showToast()
...
function dismissAllToasts() {
    var container = document.getElementById('toast-container');
    if (!container) return;
    Array.prototype.slice.call(container.children).forEach(dismissToast);
}
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        ...
        if (toastContainer && toastContainer.lastElementChild) {
            dismissToast(toastContainer.lastElementChild);
        }
    }
});
...
// Body scroll lock while a modal is open per AI.md PART 16 "Modals".
// The dialog 'close' event does not bubble, so the release listener is
// registered in the capture phase on document.
...
document.addEventListener('close', function(e) {
    if (e.target && e.target.tagName === 'DIALOG') unlockBodyScroll();
}, true);
...
(function() {
    ...
    function normalize(list) {
        ...
        return (list || []).map(function(f) {
            return {
                id: f.id,
                url: f.url,
                title: f.title || i18n.searchCardUntitled || 'Untitled',
    ...
    function add(video) {
        ...
        return fetchAPI('/v1/favorites', {
            ...
            body: JSON.stringify({
                url: video.url,
                title: video.title || i18n.searchCardUntitled || 'Untitled',
...
(function() {
    ...
    function hydrateServerResults(minDuration) {
        ...
        if (countEl) countEl.textContent = fmtI18n(getSearchI18n().resultsCount, allResults.length);
        ...
        announce(fmtI18n(getSearchI18n().resultsCount, allResults.length));
    ...
    function streamResults(minDuration) {
        ...
        eventSource.onmessage = function(event) {
            ...
            if (data.done && data.engine === 'all') {
                ...
                if (timeContainer) timeContainer.textContent = fmtI18n(getSearchI18n().resultsTime, elapsed);
                updateSearchStatus();
                hideSearchElement('status-bar');
                ...
                if (allResults.length === 0) {
                    ...
                    if (pendingGridSwap) {
                        ...
                        if (staleCountEl) staleCountEl.textContent = fmtI18n(getSearchI18n().resultsCount, 0);
                    }
                    showNoResultsMessage();
                    hasMoreResults = false;
            ...
            if (data.result && data.result.title) {
                ...
                if (firstResult) {
                    ...
                    if (pendingGridSwap) {
                        ...
                        if (oldCountEl) oldCountEl.textContent = fmtI18n(getSearchI18n().resultsCount, 0);
                    }
                }
    ...
    function fallbackToJSONSearch(minDuration) {
        ...
        .catch(function(err) {
            isSearching = false;
            var i18n = getAppI18n();
            var searchFailedMsg = i18n.searchFailed || 'Search failed - check your connection';
            if (pendingGridSwap) {
                // Background refetch failed - the still-visible old results
                // are still correct, leave them in place; just notify.
                pendingGridSwap = false;
                hideSearchElement('status-bar');
                showToast(searchFailedMsg, 'error');
                return;
            }
            var loadingEl = document.getElementById('initial-loading');
            if (loadingEl) {
                var connErrP = document.createElement('p');
                connErrP.appendChild(document.createTextNode((i18n.searchConnectionError || 'Connection error.') + ' '));
                var retryBtn = document.createElement('button');
                retryBtn.type = 'button';
                retryBtn.setAttribute('data-action', 'reload');
                retryBtn.className = 'retry-btn';
                retryBtn.textContent = i18n.retry || 'Retry';
                connErrP.appendChild(retryBtn);
                loadingEl.innerHTML = '';
                loadingEl.appendChild(connErrP);
            }
            showToast(searchFailedMsg, 'error');
            updateSearchStatus();
        });
    ...
    function addResultCard(r) {
        ...
        card.setAttribute('aria-label', r.title || i18n.searchCardVideoResultLabel || 'Video result');
        ...
        if (hasPreview) {
            ...
            if (isTouchDevice) {
                html += '<div class="swipe-hint">' + escapeHtmlUtil(i18n.searchCardSwipePreview || 'Swipe to preview') + '</div>';
            }
        }
        ...
        html += '<summary aria-label="' + escapeHtmlUtil(i18n.searchCardVideoOptions || 'Video options') + '"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></summary>';
        html += '<div class="card-menu" role="menu">';
        html += '<button type="button" class="card-menu-item" data-action="newtab" data-url="' + escapeHtmlUtil(r.url) + '"><span>' + escapeHtmlUtil(i18n.searchCardOpenNewTab || 'Open in new tab') + '</span></button>';
        html += '<button type="button" class="card-menu-item" data-action="copy" data-url="' + escapeHtmlUtil(r.url) + '"><span>' + escapeHtmlUtil(i18n.searchCardCopyLink || 'Copy link') + '</span></button>';
        html += '<button type="button" class="card-menu-item" data-action="favorite" data-video=\'' + escapeHtmlUtil(JSON.stringify({url: r.url, title: r.title || i18n.searchCardUntitled || 'Untitled', thumbnail: r.thumbnail || '', source: r.source || ''})) + '\'><span>' + escapeHtmlUtil(i18n.favoritesAdd || 'Add to favorites') + '</span></button>';
        if (hasDownload) {
            html += '<button type="button" class="card-menu-item" data-action="newtab" data-url="' + escapeHtmlUtil(downloadUrl) + '"><span>' + escapeHtmlUtil(i18n.searchCardDownload || 'Download') + '</span></button>';
        }
        html += '</div></details>';

        html += '<div class="info">';
        html += '<h3><a href="' + escapeHtmlUtil(r.url) + '"' + targetAttr + ' rel="noopener noreferrer nofollow">' + escapeHtmlUtil(r.title || i18n.searchCardUntitled || 'Untitled') + '</a></h3>';
        html += '<div class="meta"><span class="source">' + escapeHtmlUtil(r.source_display || r.source || '') + '</span>';
        if (r.views) html += '<span>' + escapeHtmlUtil(r.views) + ' ' + escapeHtmlUtil(i18n.timeViews || 'views') + '</span>';
        html += '</div></div>';
...
document.addEventListener('click', function(e) {
    ...
    var linkCopiedMsg = i18n.searchCardLinkCopied || 'Link copied to clipboard';
    var favAddedMsg = i18n.favoritesAdded || 'Added to favorites';
    var favRemovedMsg = i18n.favoritesRemoved || 'Removed from favorites';
    var favAddLabel = i18n.favoritesAdd || 'Add to favorites';
    var favRemoveLabel = i18n.favoritesRemove || 'Remove from favorites';
    ...
    if (action === 'newtab') {
        ...
        navigator.clipboard.writeText(url).then(function() {
            showNotification(linkCopiedMsg, 'success');
        }).catch(function() {
        ...
        window.Vidveil.Favorites.toggle(videoData).then(function(added) {
            showNotification(added ? favAddedMsg : favRemovedMsg, added ? 'success' : 'info');
            if (span) {
                span.textContent = added ? favRemoveLabel : favAddLabel;
            }
        });
    }
...
document.addEventListener('toggle', function(e) {
    ...
    if (favBtn) {
        ...
        window.Vidveil.Favorites.ensureLoaded().then(function() {
            favBtn.textContent = window.Vidveil.Favorites.isFavorite(videoData.url) ? (toggleI18n.favoritesRemove || 'Remove from favorites') : (toggleI18n.favoritesAdd || 'Add to favorites');
        });
    }
}, true);
...
// Substitutes the single placeholder verb in a translated string (the same
// %d/%s the Go side formats server-side) with a client-side value, so the
// count/time labels stay translated while results stream in. Falls back to
// the bare value when the key is missing from the #search-i18n payload.
function fmtI18n(tpl, value) {
    if (!tpl) return String(value);
    return tpl.replace(/%[dsv]/, String(value));
}
...
function showNoResultsMessage() {
    ...
    // Rebuild the documented empty-state structure (icon + title + message)
    // so the client-rendered state matches the server-rendered one exactly.
    el.textContent = '';
    ...
    title.textContent = i18n.noResults || '';
    var hint = document.createElement('p');
    hint.className = 'empty-state-message';
    hint.textContent = i18n.noResultsHint || '';
    el.appendChild(icon);
...
function renderFavoritesGrid(list) {
    ...
    var untitledLabel = i18n.untitled || 'Untitled';
    ...
    list.forEach(function(f) {
        ...
        var html = '<a href="' + escapeHtmlUtil(f.url) + '" rel="noopener noreferrer nofollow" aria-label="' +
            escapeHtmlUtil((f.title || untitledLabel) + ' - ' + (f.source || '')) + '">';
        html += '<img src="/api/v1/proxy/thumbnails?url=' + encodeURIComponent(f.thumbnail || '') + '" alt="' +
            escapeHtmlUtil(f.title || untitledLabel) + '" loading="lazy">';
        html += '<div class="video-info"><h3 class="video-title">' + escapeHtmlUtil(f.title || untitledLabel) +
...
// AI.md PART 16 "Buttons" - single-submit-only. Any submit button carrying
// data-loading-text is locked at its current width, disabled and relabelled the
// moment its form submits, so a double click can never post twice. Enhancement
// only: without JS the form still posts normally, just without the lock.
// Re-enabled by restoreSubmitButton() when a JS-intercepted request settles.
...
function restoreSubmitButton(btn) {
    if (!btn || !btn.dataset.idleText) return;
...
document.addEventListener('submit', function(e) {
    ...
    if (form && form.querySelectorAll) {
        var pending = form.querySelectorAll('button[data-loading-text]');
        for (var i = 0; i < pending.length; i++) {
            lockSubmitButton(pending[i]);
        }
    }
    var el = e.target.closest('[data-action]');
    if (!el) return;
...
// ============================================================================
// JS-only controls. Elements marked data-js-only ship hidden in the server
// -rendered HTML and are revealed here, so a no-JS visitor never sees a
// control that cannot work (e.g. the error page "Go Back" button).
// ============================================================================
(function() {
    onReady(function() {
        var controls = document.querySelectorAll('[data-js-only]');
        for (var i = 0; i < controls.length; i++) {
            controls[i].removeAttribute('hidden');
        }
    });
})();
...
// Optional enhancement per AI.md PART 16 — mirror the browser's native
// validationMessage into the styled .field-error element. Styling and
// submit-blocking already work with JS disabled via HTML5 + :user-invalid,
// so this only adds the message text. The 'invalid' and 'blur' events do not
// bubble, which is why these listeners are registered in the capture phase.
(function() {
    'use strict';

    var FIELD_SELECTOR = '.form-group input, .form-group select, .form-group textarea';
    ...
    // Validate on blur only — never while the user is still typing.
    document.addEventListener('blur', function(e) {
        var el = e.target;
        if (!isField(el) || !el.checkValidity) return;
        if (el.checkValidity()) {
            clearError(el);
        } else {
            showError(el);
    ...
    // Clear the mirrored message as soon as the field becomes valid again.

...
func NewPornMDEngine(cfg *config.Config, torClient *tor.Client) *PornMDEngine {
	return &PornMDEngine{
		...
		parser:     parsers.NewPornMDParser(),
	}
}

// Search performs a search on PornMD
func (e *PornMDEngine) Search(ctx context.Context, query string, page int) ([]models.Result, error) {
...
func (e *PornMDEngine) convertToResult(item *parsers.VideoItem) models.Result {
	desc := item.Quality
	if item.Description != "" {
		if desc != "" {

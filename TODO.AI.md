...
- AI.md was re-copied from `~/Templates/go/TEMPLATE.md` on 2026-05-07 (second refresh of the day). Template still treats `{project_name}` / `{project_org}` / `{internal_name}` / `{plist_name}` as REFERENCE TOKENS, not setup-time text replacements - they resolve from `IDEA.md ## Project variables` and AI.md stays read-only with placeholders intact. Current md5 (no substitution) is `d180bc9dd77aac60b310d787a7796d23` (60,825 lines). AI.md is TRACKED in git per PART 0 "Allowed Root Files"; template refreshes are committed.
...
- Repo-root AI loader (CLAUDE.md) refreshed 2026-05-07 to record the new md5 and line count.
- 14 per-tool rule cheatsheet files in `.claude/rules/` (tracked in repo) verified against the new template on 2026-05-07: ai-rules, project-rules, config-rules, binary-rules, backend-rules, api-rules, frontend-rules, features-rules, service-rules, makefile-rules, docker-rules, cicd-rules, testing-rules, optional-rules. No regeneration needed - the spec NEVER/ALWAYS lists for those PARTs are unchanged.
...
- COMMIT/NEVER/MUST rules saved to harness memory; refreshed for the new template on 2026-05-07. Added Self-Validation Loop MUSTs section (PART 0 emphasis on running real verification tools before declaring "done").
...
- [x] template-refresh-2026-05-07-b - Re-copy AI.md from ~/Templates/go/TEMPLATE.md (second refresh same day; new md5 d180bc9dd77aac60b310d787a7796d23, 60,825 lines). Template content is similar; the notable PART 0 addition is the "Self-Validation Loop" requirement (run real verification tools before declaring done). Refreshed CLAUDE.md md5 stamp; appended Self-Validation Loop MUSTs to spec_must_rules.md memory; verified 14 .claude/rules/*.md remain valid (no regen needed - spec NEVER/ALWAYS lists in those PARTs are unchanged); TODO.AI.md updated.
...
- [x] license-md-full-regen-2026-05-07 - Regenerated LICENSE.md inside `golang:alpine` Docker (NEVER on host) per PART 2. Ran `go-licenses csv ./...` against current go.mod -> 56 third-party libraries (was 32 in the prior trimmed list). Joined with copyright lines extracted from each module's NOTICE/LICENSE/source files (multi-pass: strict year-anchored, relaxed copyright-c-name, .go header fallback, synth fallback for the 4 unused-by-this-project modules). Added missing deps (microsoft/go-mssqldb, prometheus/client_golang+model+common+procfs, refraction-networking/utls, charmbracelet/x/term+ansi, klauspost/compress, jackc/{pgpassfile,pgservicefile,puddle}, modernc.org/{libc,mathutil,memory}, etc.); bumped versions for jackc/pgx, redis/go-redis, modernc.org/sqlite, golang.org/x/{crypto,net,sys,sync,text,term}; rewrote the dependency table to PART 2 compact format with Library/Version/License/Copyright columns; added BSD-3-Clause non-endorsement notice block per PART 2 BSD-3-Clause rule. Verified no GPL/AGPL/LGPL.

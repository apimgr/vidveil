...
// frozenInternalName is {internal_name} per AI.md PART 2 — frozen forever and
// used for every on-disk service identity, never the renamed binary name.
const frozenInternalName = "vidveil"
...
func NewServiceManager(appName, binaryPath, configDir, dataDir string) *ServiceManager {
	...
	return newServiceManagerWithOrg(appName, "apimgr", frozenInternalName, binaryPath, configDir, dataDir)
}
...
func (sm *ServiceManager) Disable() error {
	...
	case "linux":
		if sm.hasSystemd() {
			return exec.Command("systemctl", "disable", sm.internalName).Run()
		}
		// OpenRC: remove the service from the default runlevel
		if sm.hasOpenRC() {
			return exec.Command("rc-update", "del", sm.internalName, "default").Run()
		}
		// SysVinit: turn off the runlevel links via the host's runlevel manager
		if sm.hasSysVInit() {
			if _, err := exec.LookPath("update-rc.d"); err == nil {
				return exec.Command("update-rc.d", sm.internalName, "disable").Run()
			}
			if _, err := exec.LookPath("chkconfig"); err == nil {
				return exec.Command("chkconfig", sm.internalName, "off").Run()
			}
		}
		if sm.hasRunit() {
			// Runit: remove symlink from /var/service
			return os.Remove(fmt.Sprintf("/var/service/%s", sm.internalName))
		}
	...
	case "freebsd", "openbsd", "netbsd":
		...
		return exec.Command("sysrc", fmt.Sprintf("%s_enable=NO", sm.internalName)).Run()
	case "windows":
		// Windows: set service start type to disabled
		return exec.Command("sc", "config", sm.internalName, "start=", "disabled").Run()
	default:
		return fmt.Errorf("unsupported operating system: %s", runtime.GOOS)
	}
}
...
func (sm *ServiceManager) GetServiceStatus() (string, error) {
	...
	case "linux":
		if sm.hasSystemd() {
			out, err := exec.Command("systemctl", "status", sm.internalName).CombinedOutput()
			if err != nil {
...
func (sm *ServiceManager) runServiceCommand(action string) error {
	...
	case "linux":
		if sm.hasSystemd() {
			return exec.Command("systemctl", action, sm.internalName).Run()
		}
		if sm.hasOpenRC() {
			return exec.Command("rc-service", sm.internalName, action).Run()
		}
		if sm.hasRunit() {
			return exec.Command("sv", action, sm.internalName).Run()
		}
		return exec.Command("service", sm.internalName, action).Run()
	case "darwin":
		// Per AI.md PART 24: plist path uses {plist_name}
		plistPath := fmt.Sprintf("/Library/LaunchDaemons/%s.plist", sm.plistName)
		switch action {
	...
	case "windows":
		switch action {
		case "start":
			return exec.Command("sc", "start", sm.internalName).Run()
		case "stop":
			return exec.Command("sc", "stop", sm.internalName).Run()
		case "restart":
			exec.Command("sc", "stop", sm.internalName).Run()
			return exec.Command("sc", "start", sm.internalName).Run()
		}
	}
...
`, sm.appName, sm.projectOrg, sm.appName, sm.binaryPath,
...
// createDarwinUser creates the hidden system user/group on macOS using dscl
// per AI.md PART 23. The ID comes from the macOS safe range (200-399).
func (sm *ServiceManager) createDarwinUser() error {
	...
	// Per AI.md PART 23: macOS uses its own 200-399 safe range, not the Linux range
	id, err := findAvailableMacOSSystemID()
	if err != nil {
		return err
	}

	return createMacOSServiceUser(sm.user, id, sm.dataDir)
}

// findAvailableMacOSSystemID returns a free UID/GID from the macOS safe range
// per AI.md PART 23: start at 399, work down to 200, skip reserved IDs and any
// ID already taken by an existing user or group.
...
// createMacOSServiceUser creates the group and the hidden, shell-less service
// user with matching UID/GID per AI.md PART 23.
func createMacOSServiceUser(name string, id int, homeDir string) error {
	...
	commands := [][]string{
		...
		{"dscl", ".", "-create", groupPath, "PrimaryGroupID", strconv.Itoa(id)},
		{"dscl", ".", "-create", groupPath, "Password", "*"},
		{"dscl", ".", "-create", userPath},
		{"dscl", ".", "-create", userPath, "UniqueID", strconv.Itoa(id)},
		{"dscl", ".", "-create", userPath, "PrimaryGroupID", strconv.Itoa(id)},
		{"dscl", ".", "-create", userPath, "UserShell", "/usr/bin/false"},
		{"dscl", ".", "-create", userPath, "RealName", name + " service account"},
...
load_rc_config $name
run_rc_command "$1"
`, sm.internalName, sm.internalName, sm.binaryPath)
...
func (sm *ServiceManager) installWindows() error {
	...
	account := fmt.Sprintf("NT SERVICE\\%s", sm.internalName)

	// Install service using sc.exe
	cmd := exec.Command("sc", "create", sm.internalName,
	...
	// Set description
	exec.Command("sc", "description", sm.internalName, sm.description).Run()

	// Set failure recovery
	exec.Command("sc", "failure", sm.internalName, "reset=", "86400", "actions=", "restart/5000/restart/10000/restart/30000").Run()
...
func (sm *ServiceManager) uninstallWindows() error {
	...
	sm.Stop()
	exec.Command("sc", "delete", sm.internalName).Run()
	fmt.Printf("Service %s uninstalled\n", sm.internalName)
	return nil
}

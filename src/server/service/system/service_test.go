...
// TestNewServiceManager_DerivedFields verifies that user and group are the
// frozen internal_name (never the possibly-renamed appName argument) per
// AI.md service-rules.md: "Always use {internal_name} (frozen) for service
// identity ... never {project_name}", and that description is derived from
// appName.
func TestNewServiceManager_DerivedFields(t *testing.T) {
	sm := NewServiceManager("myapp", "/bin/myapp", "/etc/myapp", "/var/myapp")

	if sm.user != frozenInternalName {
		t.Errorf("user = %q, want %q", sm.user, frozenInternalName)
	}
	if sm.group != frozenInternalName {
		t.Errorf("group = %q, want %q", sm.group, frozenInternalName)
	}
	if sm.description == "" {

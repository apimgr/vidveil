...
// Non-empty data-preview on img populates PreviewURL (made absolute).
// Covers xvideos.go:84-86.
func TestXVideosParse_PreviewURL_MadeAbsolute(t *testing.T) {
	p := NewXVideosParser()
	html := `<div class="thumb-block">
		<a href="/video/pv1" title="Preview URL Test">
			<img src="thumb.jpg" data-preview="//cdn.xvideos.com/preview.mp4">
		</a>
	</div>`
	item := p.Parse(newDoc(html).Find("div.thumb-block"))
	if item == nil {
	...
	if item.PreviewURL == "" {
		t.Error("PreviewURL must not be empty when data-preview is present")
	}
}

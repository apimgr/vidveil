...
- Disabled engines (source retained, not registered in `InitializeEngines()`, no bang assigned): `pornmd`, `fourtube`, `fux`, `empflix`, `gotporn`, `pornerbros`, `pornhd`, `tubegalore` (site returns HTTP 403 Cloudflare challenge with no available bypass), and `motherless` (site's aggressive TLS fingerprinting blocks automated requests). These do not count toward the 33 active engines above.

...
func TestPornMDConvertToResult_DescriptionWithQuality(t *testing.T) {
	...
	r := e.convertToResult(item)
	if r.Quality != "HD" {
		t.Errorf("PornMD result should carry Quality directly, got %q", r.Quality)
	}
	if r.Description != "some desc" {
		t.Errorf("PornMD description should be passed through unmodified, got %q", r.Description)
	}
}

...
func TestBuildTorrc_NilConfigDefaultsAccepted(t *testing.T) {
	got := buildTorrc(nil, "/tmp/hs", 64500, "/tmp/log/tor.log")
	if got == "" {
...
func TestBuildTorrc_UseNetworkEnablesSocks(t *testing.T) {
	...
	cfg.UseNetwork = true
	got := buildTorrc(&cfg, "/tmp/hs", 64500, "/tmp/log/tor.log")
...
func TestBuildTorrc_HiddenServiceDeclared(t *testing.T) {
	cfg := config.DefaultTorConfig()
	got := buildTorrc(&cfg, "/tmp/hs", 64500, "/tmp/log/tor.log")
	if !strings.Contains(got, "HiddenServiceDir /tmp/hs") {
		t.Errorf("buildTorrc: expected HiddenServiceDir /tmp/hs, got:\n%s", got)
	}
	if !strings.Contains(got, "HiddenServicePort 80 127.0.0.1:64500") {
		t.Errorf("buildTorrc: expected HiddenServicePort 80 127.0.0.1:64500, got:\n%s", got)
	}
	if !strings.Contains(got, "HiddenServiceExportCircuitID haproxy") {
		t.Errorf("buildTorrc: expected HiddenServiceExportCircuitID haproxy, got:\n%s", got)
	}
	if !strings.Contains(got, "ControlPort 127.0.0.1:auto") {
		t.Errorf("buildTorrc: expected ControlPort 127.0.0.1:auto, got:\n%s", got)
	}
}
...
func TestEnsureTorrc_ExistingFileOverwritten(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "torrc")
	original := "# original\n"
	...
	created, err := ensureTorrc(path, newContent)
	if err != nil {
		t.Fatalf("ensureTorrc: %v", err)
	}
	if created {
		t.Error("ensureTorrc: expected created=false for pre-existing file")
	}
	data, _ := os.ReadFile(path)
	if string(data) != newContent {
		t.Errorf("ensureTorrc: expected file to be overwritten with new content: got %q, want %q", string(data), newContent)
	}
}
